...
from wasmer_app.models import DeployedApp, Plan, User


class GraphQLUserTests(TransactionTestCase):
    def setUp(self):
        self.user_hobby = User.objects.create(
            id="u_test_hobby", username="alice", plan=Plan.HOBBY
        )
        self.user_pro = User.objects.create(
            id="u_test_pro", username="alice", plan=Plan.PRO
        )

        self.deployed_app = DeployedApp.objects.create(
            id="app_test_pro", owner=self.user_pro, active=True
        )
    ...
    async def test_fetch_user_node(self):
        ...
        self.assertEqual(
            response.data["node"],
            {
                "id": self.user_hobby.id,
                "username": self.user_hobby.username,
                "plan": self.user_hobby.plan.value,
            },
        )
    ...
    async def test_fetch_deployed_app(self):
        ...
        self.assertEqual(
            response.data["node"], {"id": self.deployed_app.id, "active": True}
        )

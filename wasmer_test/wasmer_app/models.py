...
class User(models.Model):
    id = models.CharField(primary_key=True, default=user_id, editable=False, max_length=50)
...
class DeployedApp(models.Model):
    id = models.CharField(primary_key=True, default=deployed_app_id, editable=False, max_length=50)
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name='apps')
    active = models.BooleanField(default=True)


class ProviderCredential(models.Model):
    id = models.CharField(primary_key=True, default=provider_credential_id, editable=False, max_length=50)
...
class Email(models.Model):
    id = models.CharField(primary_key=True, default=email_id, editable=False, max_length=35)
    deployed_app = models.ForeignKey(
        DeployedApp, on_delete=models.DO_NOTHING, related_name='emails', blank=True, db_index=True
    )

    provider_credential = models.ForeignKey(
        ProviderCredential, on_delete=models.CASCADE, related_name='emails', blank=True, null=True, db_index=True
    )
